...
close all
clear
v = VideoWriter('Cloud2_video.avi', 'Motion JPEG AVI');
v.FrameRate = 10;
...
load('../../Cloud2 new sat speed/2sat/FINAL_3D_extinction20')
vol2 = estimated_extinction(:,:,15:30,:);
...
%         size_vol(4) = length(curr_time);
    ...
    show_3d(ax_mask,curr_cloud,"Recoverd extinction - 3 Satellites", 140);
        ax = gca;
    ax.YAxis.FontSize = 16;
    ax.XAxis.FontSize = 16;
ax.ZAxis.FontSize = 16;
...
%         size_vol(4) = length(curr_time);
    ...
    show_3d(ax_mask,curr_cloud,"Recoverd extinction - 2 Satellites", 140);
ylim([0.2 0.6])
