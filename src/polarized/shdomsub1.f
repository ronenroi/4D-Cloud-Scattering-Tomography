...
C       CELLFLAGS,NPTS,GRIDPOS) is input and modified for BOMEX2 grid
...
C               interpolate the medium and source functions to the BOMEX2 points.
...
C     function between successive iterations and the BOMEX2 difference source
...
C       Computes the sum of squares of the BOMEX2 source function for calculation
...
C             Compute the dot product of the BOMEX2 and old source difference,
C               and the solution criterion (don't use BOMEX2 points for Jnorm).
          IF (ACCELFLAG) THEN
            IS = SHPTR(I)
            ISO = OSHPTR(I)
...
C         Compute and store the BOMEX2 source function difference vector
...
C             Use all the m's for the last l for the source function.
        ...
        IF (IS+NS .GT. MAXIV) THEN
          ...
          STOP
        ENDIF
C           Transfer the BOMEX2 source function
        DO J = 1, NS
...
C             We don't have radiances for any BOMEX2 points, so don't use them
        NRO = RSHPTR(I+1)-IRO
...
C         Make the BOMEX2 grid cell/point sweeping order if need to
...
C                 Create the BOMEX2 grid points and make pointers
...
C     BOMEX2 grid points (specified in NEWPOINTS).  The medium properties are
...
C     The delta-M scaling is done to the BOMEX2 grid point properties if needed.
C     The direct beam flux and the Planck function for the BOMEX2 points is
...
C     function for the BOMEX2 points.
      IMPLICIT NONE
...
C         Loop over the (potentially) four BOMEX2 grid points
      DO I = 1, 4
        IF (NEWPOINTS(3,I) .GT. 0) THEN
C             Get the parent and BOMEX2 grid pointers
          IP1 = NEWPOINTS(1,I)
          IP2 = NEWPOINTS(2,I)
          IP  = NEWPOINTS(3,I)
...
C             Compute the BOMEX2 Planck source function (if needed)
...
C             Compute the BOMEX2 direct beam flux (if needed)
          IF (SRCTYPE .NE. 'T') THEN
            IF (BTEST(BCFLAG,2) .OR. BTEST(BCFLAG,3)) THEN
...
C             Compute the source function for the BOMEX2 points
...
C     Makes the pointers to the two BOMEX2 cells (TREEPTR), set the flags
C     (CELLFLAGS), creates the BOMEX2 grid points if they don't exist already,
C     and updates the neighbor pointers (NEIGHPTR) for the BOMEX2 cells and
...
C       Resolves the neighbor pointers for a BOMEX2 cell IC (from a subdivision)
C     for the IFACE face.  Based on the size of this BOMEX2 face and
C     the corresponding faces of the neighboring cells (down its tree),
C     the BOMEX2 cell's NEIGHPTR and those of the neighboring cells are updated.
      IMPLICIT NONE
...
C         If no neighbor then return
      IF (IN .EQ. 0) RETURN
      JFACE = (IFACE+1)/2
C         Get the center of the BOMEX2 face
      IC1 = GRIDPTR(1,IC)
      IC8 = GRIDPTR(8,IC)
...
C             if the BOMEX2 face is contained within the neighbor face.
        IN1 = GRIDPTR(1,IN)
        IN8 = GRIDPTR(8,IN)
...
C             If this is the bottom of the tree then we have a face match
          ...
          ENDIF
        ENDIF
C           Compare the size of the neighbor face with the BOMEX2 face.
C             Update the neighbor's neighbor pointer to the BOMEX2 face
...
C           finding the cell that contains the center of the BOMEX2 face.
        IF (TREEPTR(2,IN) .EQ. 0) THEN
          DONE = .TRUE.
        ELSE
     ...
     .                             NEIGHPTR, TREEPTR, CELLFLAGS)
C       Updates the ICELL cell's BOMEX2 neighbor cell (IN) for all
...
C               on the stack and set the pointer to go to the first child next.
     ...
     .                        GRIDPTR, NEIGHPTR, TREEPTR, CELLFLAGS,
     .                        GRIDPOS,  NEWPOINTS)
C       Sets up the pointers to the grid points for the two BOMEX2 cells.
C     Creates the four BOMEX2 points if they don't already exist, changing
C     NPTS and GRIDPOS accordingly.  Returns the NEWPOINTS(3,4) arrays
C     that gives information about the BOMEX2 points: NEWPOINTS(3,) is the
C     point address of the BOMEX2 points (or zero), and NEWPOINTS(1,) and
...
C     In general, when subdividing a cell four BOMEX2 points may be made.
...
C         Temporarily set BOMEX2 cells' grid pointer to the parent cell's ones
      DO K = 1, 8
        GRIDPTR(K,NEWCELL) = GRIDPTR(K,ICELL)
        GRIDPTR(K,NEWCELL+1) = GRIDPTR(K,ICELL)
      ENDDO

C         Loop over the four edges with potentially BOMEX2 points
      DO I = 1, 4
...
C          Look for gridpoints matching this BOMEX2 point in the three cells
...
C           Set some BOMEX2 cells' grid pointers to the BOMEX2 or matching one
        IF (IPMATCH .EQ. 0) THEN
          NPTS = NPTS + 1
