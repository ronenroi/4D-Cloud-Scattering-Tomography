...
close all
clear
v = VideoWriter('Cloud2_3sat_video.avi', 'Motion JPEG AVI');
v.FrameRate = 10;


load('../../Cloud2 new sat speed/3sat/GT_3D_extinction.mat')
vol = gt_extinction(:,:,:,15:30);
load('../../Cloud2 new sat speed/3sat/FINAL_3D_extinction20')
vol1 = estimated_extinction(:,:,15:30,:);
...
%         ax_mask = subplot(1,3,1);
    ...
    show_3d(ax_mask,curr_cloud,"Ground-truth extinction", 140);
        ax = gca;
...
%         size_vol(4) = length(curr_time);
    ...
    show_3d(ax_mask,abs(curr_cloud1 ),"Recoverd extinction - 3 Satellites", 140);
        ax = gca;
    ax.YAxis.FontSize = 16;
    ax.XAxis.FontSize = 16;
ax.ZAxis.FontSize = 16;
...
ax_mask = subplot(1,3,3);
    ...
    show_3d(ax_mask,abs(curr_cloud - curr_cloud1),"$|$ $ ${\boldmath $\beta$}$^{\rm true}$ $-${ {\boldmath ${\hat \beta}$}} $|$  - 3 Satellites", 80);

tex = annotation('textbox', [0.45,0.141825477864104,0.146330275229358,0.058174522135896], 'string', ['Time=' ...
...
c = colorbar;
colormap(ax,'cool')
c.Position = [0.928,0.219317903459121,0.020668285662889,0.612166499498496];
c.Label.String = 'Extinction [1/km]';
c.FontSize=20;
c.Label.Position = [0.370175462020071,89.2519714969328,0];
c.Label.Rotation = 0;
ylim([0.2 0.6])
