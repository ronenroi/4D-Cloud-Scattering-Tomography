...
class RteSolver(object):
    ...
    def set_scene(self, scene_params):
        ...
        if scene_params.surface.type == 'Lambertian':
            ...
            raise NotImplementedError('Only Lambertian and Ocean surfaces are currently supported')
    ...
    def set_medium(self, medium):
        ...
        for i, scatterer in enumerate(medium.scatterers.values()):
            ...
            if isinstance(scatterer, shdom.MicrophysicalScatterer) or isinstance(scatterer, shdom.MultispectralScatterer):
                optical_scatterer = scatterer.get_optical_scatterer(self.wavelength)
            else:
                optical_scatterer = scatterer
            resampled_scatterer = optical_scatterer.resample(medium.grid)
    ...
    def init_memory(self):
        ...
        debug_flag = False
        if(debug_flag):# vadim just want to see when the memory is initialized.
            print(" --rte:init_memory--")
        debug_memory_flag = False

        # Vadim add this print when he whanted to see how much memory is beeing alocated.
        ...
        self._maxir = self._maxiv + self._maxig # new parameter interduced by vadim,
        # TODO - use it in the fortran code.
        ...
        if(debug_memory_flag):
            # Vadim add this to monitor memory usage
    ...
    def init_solution(self):
        ...
        debug_flag = False
        if(debug_flag):# vadim just whant to see when there is rte solution
            print(" --rte:init_solution--")
        # TODO- include the maxir=self._maxir, in the core.init_solution function.
...
class RteSolverArray(object):
    ...
    def add_solver(self, rte_solver):
        ...
        if isinstance(rte_solver, RteSolver):
            ...
            self._num_solvers += 1
            self._rte_type_list.append(rte_solver.type)
        else:
