...
close all
clear
v = VideoWriter('Cloud2_2sat_video.avi', 'Motion JPEG AVI');
v.FrameRate = 10;
...
%         size_vol(4) = length(curr_time);
    ...
    show_3d(ax_mask,abs(curr_cloud1 ),"Recoverd extinction - 2 Satellites", 140);
        ax = gca;
    ax.YAxis.FontSize = 16;
    ax.XAxis.FontSize = 16;
ax.ZAxis.FontSize = 16;
...
ax_mask = subplot(1,3,3);
    ...
    show_3d(ax_mask,abs(curr_cloud - curr_cloud1),"$|$ $ ${\boldmath $\beta$}$^{\rm true}$ $-${ {\boldmath ${\hat \beta}$}} $|$  - 2 Satellites", 80);
