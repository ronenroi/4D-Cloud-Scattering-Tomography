...
C          IF (SRCTYPE .NE. 'T' .AND. DELTAM) THEN
C            DO IPA = 1, NPART
...
C              ENDIF
C              IF (W.EQ.0.0) CYCLE
C              IF (NUMPHASE .GT. 0) THEN
C                IPH = IPHASE(IP,IPA)
C              ELSE
C                IPH = IP
C              ENDIF
CCC            DA = ALBEDO(IP,IPA)*DIRFLUX(IP)*SECMU0*W
C            J = 1
C            DO L = 0, ML
C              ME = MIN(L,MM)
C              MS = -ME
...
C                JT = J
C                DO M = MS, ME
C                  SRCEXT8(N) =SRCEXT8(N)-A1*YLMDIR(J)*YLMSUN(J)
C                  J = J + 1
C                ENDDO
C              ENDIF
C            ENDDO
CC               Then add in the single scattering contribution for the
CC               original unscaled phase function.
C            IF (NUMPHASE .GT. 0) THEN
C              SRCEXT8(N) = SRCEXT8(N) + DA*SINGSCAT(IPH)
C            ELSE
C              F = LEGEN(ML+1,IPH)
C              DO L = 0, NLEG
C                IF (L .LE. ML) THEN
C                  A = DA*(LEGEN(L,IPH) + F/(1-F))
C                ELSE
C                  A = DA*LEGEN(L,IPH)/(1-F)
C                ENDIF
C                SRCEXT8(N) = SRCEXT8(N) + A*SUNDIRLEG(L)
C              ENDDO
C            ENDIF
C            ENDDO
C          ENDIF
C
C          SRCEXT8(N) = SRCEXT8(N)*EXT
C          EXTINCT8(N) = EXT
C        ENDIF
C      ENDDO
C
